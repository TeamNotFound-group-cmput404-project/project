...
class LoginView(View):
    def get(self, request):
        ...
        return render(request, 'Iconicity/login.html', { 'form':  PickyAuthenticationForm })

    def post(self,request):
        form = PickyAuthenticationForm(request, data=request.POST)
        if form.is_valid():
            try:
                form.clean()
...
def getPosts(user, visibility="PUBLIC"):
    assert visibility in ["PUBLIC","FRIENDS","UNLISTED"], "Not valid visibility for posts, check getPosts method in views.py"
    if visibility == "PUBLIC":
        ...
        return list(Post.objects.filter(author=user.id, visibility="UNLISTED"))
...
def mypost(request):
    ...
    postList += getPosts(request.user, visibility="UNLISTED")
